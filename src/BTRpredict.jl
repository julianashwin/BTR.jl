"""
Out-of-sample prediction for Bayesian Topic Regression
...
"""
Out-of-sample prediction for Bayesian Topic Classsification
    Takes an (unseen) BTCCorpus and a BTCModel as arguments
    Outputs a BTCPrediction object
"""
function BTCpredict(btccrps::DocStructs.BTCCorpus, btcmodel::BTCModel)::BTCPrediction
    ...
    # Extract the estimated parameters and priors
    α::Float64 = opts.α
    β::Array{Float64,2} = btcmodel.β
    ω::Array{Float64,1} = btcmodel.ω
    ...
    # Extract the data
    docs::Array{DocStructs.BTCParagraphDocument,1} = btccrps.docs
    ...
    # Create regressors for prediction
    y::Array{Int64,1} = vcat(getfield.(btccrps.docs, :y)...)
    x::Array{Float64,2} = vcat(getfield.(btccrps.docs, :x)...)
    Z_bar = Matrix(transpose(Z_bar_avg))
    Z_bar ./= sum(Z_bar, dims=2)
    # Placeholders to store the document-topic and topic-word distributions
    inter_effects = zeros(btccrps.N,(ntopics*length(interactions)))
    for jj in 1:length(interactions)
        col_range::UnitRange{Int64} = ((jj*ntopics-ntopics+1):(jj*ntopics))
        inter_effects[:,col_range] = Z_bar.*vec(x[:,interactions[jj]])
    end
    regressors::Array{Float64,2} = hcat(Z_bar, inter_effects, x[:,nointeractions])
    @assert size(regressors) == (btccrps.N, length(ω)) "Regressors and ω don't match"

    # Renormalise θ and β to eliminate any numerical errors
    ylin::Array{Float64,1} = regressors*ω

    e_ylin::Array{Float64,1} = exp.(ylin)
    p_pred::Array{Float64,1} = e_ylin./(1 .+ e_ylin)
    y_pred::Array{Int64,1} = [if x < 0.5 0 else 1 end for x in p_pred];
