...
"""
Function that performs the E-step, of sampling topic assignments, on a BTCModel object
"""
function BTCEstep(btcmodel::BTCModel)::BTCModel
    ## Extract the important parts of btrmodel
    docs::Array{DocStructs.BTCParagraphDocument,1} = btcmodel.crps.docs
    topics::Array{DocStructs.Topic,1} = btcmodel.crps.topics
    opts::BTCOptions = btcmodel.options
    ## Some useful identifiers and parameters
    ntopics::Int64 = opts.ntopics
    V::Int64 = btcmodel.crps.V
    interactions::Array{Int64,1} = opts.interactions
    nointeractions::Array{Int64,1} = setdiff(opts.xregs, opts.interactions)
    α::Float64 = opts.α
    η::Float64 = opts.η
    ## Split up ω into the relevant parts
    ω_z::Array{Float64,1} = btcmodel.ω[1:ntopics]
    ω_zx::Array{Float64,1} = btcmodel.ω[(ntopics+1):(ntopics+length(interactions)*ntopics)]
    ω_x::Array{Float64,1} = btcmodel.ω[(ntopics+length(interactions)*ntopics+1):length(btcmodel.ω)]
    ω_docspec::Array{Float64,1} = zeros(size(btcmodel.ω))
    ## Objects used as placeholders
    zprobs::Array{Float64,1} = zeros(ntopics)
    y_dd::Int64 = 0
    ...
    ## Set Z_bar and β to zero to be filled with the average across iterations
    btcmodel.Z_bar::Array{Float64,2} = zeros(size(btcmodel.Z_bar))
    btcmodel.β::Array{Float64,2} = zeros(size(btcmodel.β))
    ...
    for it in 1:(opts.E_iters+opts.burnin)
        ...
        for (dd,doc) in enumerate(docs)
            ...
            # Predicted y without the document (i.e. just x)
            ylin_minusd = dot(doc.x[:,nointeractions],ω_x)
            ...
            for para in paras
                ## What would the prediction for y be without this paragraph?
                doc.topicidcount -= para.topicidcount
                ylin_minusp = ylin_minusd + sum((ω_docspec./N_d).*doc.topicidcount)
        ...
        if it > opts.burnin
            btcmodel.β += (β.*(1/opts.E_iters))::Array{Float64,2}
            btcmodel.Z_bar += (Z_bar.*(1/opts.E_iters))::Array{Float64,2}
        end
        next!(prog)
    end ## End of E-step cycle
