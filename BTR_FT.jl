...
## Load data
df = CSV.read("/Users/julianashwin/Documents/DPhil/Clean_Data/FT/matched/clean_equities_articles.csv",
    DataFrame, threaded = false)
df = df[(.!(df.highlow.=="NA")),:]
df = df[(.!(df.abs_intra_day.=="NA")),:]
df.main_text[(df.main_text.=="NA")] .= ""
showtable(df)

df.highlow = parse.(Float64, df.highlow)
df.abs_intra_day = parse.(Float64, df.abs_intra_day)
...
fm = @formula(highlow ~ mention + abs_intra_day)
...
BTR_plot(btrmodel.β, btrmodel.ω_post, btrmodel.crps.vocab,
    plt_title = "Yelp BTR", fontsize = 10, nwords = 10, title_size = 10)
...
BTR_plot(ldamodel.β, ldamodel.ω_post, ldamodel.crps.vocab,
    plt_title = "Yelp LDA", fontsize = 10, nwords = 10, title_size = 10)
if save_files; savefig("figures/Yelp_BTR/Yelp_LDA.pdf"); end;
...
BTR_plot(slda1model.β, slda1model.ω_post, slda1model.crps.vocab,
    plt_title = "Yelp BLR + sLDA", fontsize = 10, nwords = 10, title_size = 10)
if save_files; savefig("figures/Yelp_BTR/Yelp_LR_sLDA.pdf"); end;
...
subdirectory = "/Users/julianashwin/Desktop/BTR_runs/Yelp/run_"
nruns = 20
