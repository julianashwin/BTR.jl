...
## Load data
df = CSV.read("data/comb_sample.csv", DataFrame, threaded = false)
...
ols = lm(@formula(yelp ~ sentiment + stars_av_b + stars), df)
display(ols)
...
x = Array{Float64,2}(hcat(df.sentiment,df.stars,df.stars_av_b))
y = Array{Int64,1}(df.yelp)
...
## x variables
btcopts.xregs = [1,2,3]
btcopts.interactions = Array{Int64,1}([])
